    ...
    switch (instruction.funct3) {
        0x0 => {
            switch (instruction.funct7) {
                // ADD
                0x00 => {
                    c.write(instruction.rd, c.read(instruction.rs1) +% c.read(instruction.rs2));
                },
                // SUB
                0x20 => {
                    c.write(instruction.rd, c.read(instruction.rs1) -% c.read(instruction.rs2));
        ...
        // SLL
        0x1 => {
            const shamt: u5 = @intCast(c.read(instruction.rs2) & 0x1f);
            c.write(instruction.rd, c.read(instruction.rs1) << shamt);
        },
        // SLT
        0x2 => {
            const signed_rs1: i32 = @bitCast(c.read(instruction.rs1));
            const signed_rs2: i32 = @bitCast(c.read(instruction.rs2));
            c.write(instruction.rd, if (signed_rs1 < signed_rs2) 1 else 0);
        },
        // SLTU
        0x3 => {
            c.write(instruction.rd, if (c.read(instruction.rs1) < c.read(instruction.rs2)) 1 else 0);
        },
        // XOR
        0x4 => {
            c.write(instruction.rd, c.read(instruction.rs1) ^ c.read(instruction.rs2));
        ...
        0x5 => {
            switch (instruction.funct7) {
                ...
                0x20 => {
                    const shamt: u5 = @intCast(c.read(instruction.rs2) & 0x1f);
                    const signed: i32 = @bitCast(c.read(instruction.rs1));
                    c.write(instruction.rd, @bitCast(signed >> shamt));
                },
                else => unreachable,
            }
        },
...
pub fn execute_i_alu(c: *cpu.cpu, raw: u32) void {
    ...
    switch (instruction.funct3) {
        // ADDI
        0x0 => {
            c.write(instruction.rd, c.read(instruction.rs1) +% sign_extend_12(instruction.imm));
        },
        // SLLI
        0x1 => {
        ...
        0x2 => {
            ...
            c.write(instruction.rd, if (signed_rs1 < signed_imm) 1 else 0);
        },
        // SLTIU
        0x3 => {
            c.write(instruction.rd, if (c.read(instruction.rs1) < sign_extend_12(instruction.imm)) 1 else 0);
        },
        // XORI
        0x4 => {
            c.write(instruction.rd, c.read(instruction.rs1) ^ sign_extend_12(instruction.imm));
...
pub fn execute_i_load(c: *cpu.cpu, m: *memory.memory, raw: u32) void {
    const instruction = decoder.decode_i(raw);
    const addr = c.read(instruction.rs1) +% sign_extend_12(instruction.imm);

    switch (instruction.funct3) {
        // LB
        0x0 => {
            const val: i8 = @bitCast(m.read8(addr));
            c.write(instruction.rd, @bitCast(@as(i32, val)));
        },
        // LH
        0x1 => {
        ...
        // LW
        0x2 => {
            c.write(instruction.rd, m.read32(addr));
        },
        // LBU
        0x4 => {
            c.write(instruction.rd, @as(u32, m.read8(addr)));
        },
        // LHU
        0x5 => {
            c.write(instruction.rd, @as(u32, m.read16(addr)));
        },
        else => {},
    }
...
pub fn execute_i_jalr(c: *cpu.cpu, raw: u32) void {
    ...
    switch (instruction.funct3) {
        ...
        0x0 => {
            const ret = c.pc + 4;
            c.pc = (c.read(instruction.rs1) +% sign_extend_12(instruction.imm)) & ~@as(u32, 1);
            c.write(instruction.rd, ret);
        },
        else => {},
    }
}
...
pub fn execute_s(c: *cpu.cpu, m: *memory.memory, raw: u32) void {
    ...
    switch (instruction.funct3) {
        // SB
        0x0 => {
            m.write8(addr, @truncate(c.read(instruction.rs2)));
        },
        // SH
        0x1 => {
            m.write16(addr, @truncate(c.read(instruction.rs2)));
        },
        // SW
        0x2 => {
            m.write32(addr, c.read(instruction.rs2));
        },
        else => {},
    }
...
pub fn execute_b(c: *cpu.cpu, raw: u32) void {
    ...
    switch (instruction.funct3) {
        // BEQ
        0x0 => {
            if (c.read(instruction.rs1) == c.read(instruction.rs2)) {
                c.pc = target;
            } else {
                c.pc += 4;
            }
...
pub fn execute_u_lui(c: *cpu.cpu, raw: u32) void {
    ...
    c.write(instruction.rd, @as(u32, instruction.imm) << 12);
...
pub fn execute_u_auipc(c: *cpu.cpu, raw: u32) void {
    const instruction = decoder.decode_u(raw);
    c.write(instruction.rd, c.pc +% (@as(u32, instruction.imm) << 12));
